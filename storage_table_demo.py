...
resourcegroup_name = 'skk'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'skk'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
coffeeshop.RowKey = '005'
coffeeshop.brand = 'starbucks'
coffeeshop.flavor = 'darkroast'
coffeeshop.size = 'small'
coffeeshop.price = 2.99
...
coffeeshop = Entity()
coffeeshop.PartitionKey = 'coffeeshop'
coffeeshop.RowKey = '006'
coffeeshop.brand = 'timhortins'
coffeeshop.flavor = 'pumpkinspice'
coffeeshop.size = 'large'
coffeeshop.price = 2.49
table_service.insert_entity('itemstable', coffeeshop)
print('Created entry for a Sweater...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'cars'", select='make,model,price, year')
for item in items:
